...
@loader.tds
class BasketballMod(loader.Module):
    """Basketball"""
    strings = {"name": "Basketball",
               "ball_emoji_cfg_doc": "The emoji to be sent to Telegram as a ball's. Can currently be either 🎲 or 🎯"}

    def __init__(self):
        self.config = loader.ModuleConfig("DICE_EMOJI", "'\U0001F3C0'", lambda: self.strings["ball_emoji_cfg_doc"])

    def config_complete(self):
        self.name = self.strings["name"]

    async def ballcmd(self, message):
        """Rolls a basketball (optionally with the specified value)(edited by @mpamop)"""
